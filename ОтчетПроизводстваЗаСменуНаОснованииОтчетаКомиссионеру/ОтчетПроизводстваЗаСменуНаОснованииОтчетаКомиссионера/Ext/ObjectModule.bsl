...
Процедура  ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначения, СозданныеОбъекты, ПараметрыВыполненияКоманды) Экспорт
	...
	Для каждого ТекущаяСсылка Из ОбъектыНазначения Цикл
		...
		ДокументОПС.Комментарий 		= ТекущаяСсылка.Комментарий;
		ДокументОПС.Организация 		= ТекущаяСсылка.Организация;
		ДокументОПС.Ответственный 		= ТекущаяСсылка.Ответственный;
		
		Продукция = ДокументОПС.Продукция;
		...
		ЗаполнениеДокументов.Заполнить(ДокументОПС, ТекущаяСсылка);
		
		Если НЕ ЗначениеЗаполнено(ДокументОПС.СчетЗатрат) Тогда
			ДокументОПС.СчетЗатрат   = ПланыСчетов.Хозрасчетный.ОсновноеПроизводство;
		КонецЕсли;
		
		ДокументОПС.ИспользоватьМатериалы = Истина;
