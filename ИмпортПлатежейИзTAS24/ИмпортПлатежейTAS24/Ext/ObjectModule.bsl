...
Процедура СохранитьНастройки(ПараметрыВызова) Экспорт 
...
Функция ПолучитьПоддерживаемыеРежимы() Экспорт 
	...
	ИмпортЭкспорт.Вставить("ИмпортHTTP", Истина);
...
КонецФункции //ПолучитьПоддерживаемыеРежимы

Функция ПолучитьТекущийТипФайла(ПараметрыВызова) Экспорт 
	Возврат "HTTP";
КонецФункции //ПолучитьТекущийТипФайла

Функция ПолучитьТекущуюКодировкуФайла(ПараметрыВызова) Экспорт 
	Возврат "HTTP";
КонецФункции //ПолучитьТекущуюКодировкуФайла
...
Функция ЕстьКодВБанке() Экспорт 
	Возврат Ложь;
КонецФункции //ЕстьКодВБанке

Функция СформироватьФильтрВыбораФайла(ПараметрыВызова) Экспорт 
	
	
КонецФункции //СформироватьФильтрВыбораФайла

Функция СформироватьИмяФайла(ПараметрыВызова) Экспорт 
	Возврат "HTTP";
...
//
// Параметры:
//  ПараметрыФормата - Структура - структура параметров, где
//  * ДатаДок - Дата выписки  – дата
//  * Организация - Организация - СправочникСсылка.Организации
//  * РСчет - Расчетный счет организации - строка
//  * ИмяФайла - Имя файла  - строка, не используется
//  * ТипФайла - Тип файла  - строка ("TXT"/"DBF"/"XML"), не используется
//  * ДополнительныеСвойства - Структура  - Параметры авторизации
// 
// Возвращаемое значение:
...
//
Функция ИмпортВыписки(ПараметрыФормата) Экспорт 
		
	ОтветGetAccountsКакJson = GetAccounts(
			ПараметрыФормата.ДополнительныеСвойства.access_token,
			ПараметрыФормата.idSlave,
			ПараметрыФормата.ДополнительныеСвойства.refresh_token);
			
			
	Если ЗначениеЗаполнено(ОтветGetAccountsКакJson.Получить("error_code")) Тогда
		ВызватьИсключение ОтветGetAccountsКакJson.Получить("error_code");
	КонецЕсли; 		
			
	СчетаВСистемеTAS24 = ОтветGetAccountsКакJson.Получить("accounts");
	
	СписокСчетов = Новый СписокЗначений;

	Для Индекс = 0 По СчетаВСистемеTAS24.ВГраница() Цикл		
		
		Счет = СчетаВСистемеTAS24[Индекс];                           		
		СписокСчетов.Добавить(Счет.Получить("acc_number"),Счет.Получить("currency"));
		
	КонецЦикла; 
	
	Если СписокСчетов.НайтиПоЗначению(ПараметрыФормата.РСчет) = Неопределено Тогда		
		ВызватьИсключение СтрШаблон("Счет %1 не найден в системе tas24b.ua",ПараметрыФормата.РСчет);		
	КонецЕсли; 
	
	ПараметрыЗапросаВыписок = Новый Соответствие;
	ПараметрыЗапросаВыписок.Вставить("account_number",	ПараметрыФормата.РСчет);//ПараметрыФормата.РСчет);
	ПараметрыЗапросаВыписок.Вставить("currency",		СписокСчетов.НайтиПоЗначению(ПараметрыФормата.РСчет).Представление);
	ПараметрыЗапросаВыписок.Вставить("data_begin",		Формат(ПараметрыФормата.ДатаДок,"ДФ=dd.MM.yyyy"));
	ПараметрыЗапросаВыписок.Вставить("data_end",		Формат(ПараметрыФормата.ДатаДок,"ДФ=dd.MM.yyyy"));
	ПараметрыЗапросаВыписок.Вставить("x-clientId",		СтрЗаменить(ПараметрыФормата.idSlave,Символы.НПП,""));
	ПараметрыЗапросаВыписок.Вставить("access_token",	ПараметрыФормата.ДополнительныеСвойства.access_token);
	ПараметрыЗапросаВыписок.Вставить("refresh_token",	ПараметрыФормата.ДополнительныеСвойства.refresh_token);	
	
	ОтветGetBy_accountКакJson = GetBy_account(ПараметрыЗапросаВыписок); 	
	
	Если ЗначениеЗаполнено(ОтветGetBy_accountКакJson.Получить("error_code")) Тогда		
		
		Если ЗначениеЗаполнено(ОтветGetBy_accountКакJson.Получить("message")) Тогда
			
			ВызватьИсключение ОтветGetBy_accountКакJson.Получить("message");
			
		ИначеЕсли ОтветGetBy_accountКакJson.Получить("wrong_fields") Тогда 
			
			Для каждого КлючИЗначение Из ОтветGetBy_accountКакJson.Получить("wrong_fields") Цикл				
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("Ошибка поля:%1",КлючИЗначение.Ключ);
				Сообщение.Сообщить(); 				                                            			
			КонецЦикла; 		
		КонецЕсли;      		
		ВызватьИсключение ОтветGetBy_accountКакJson.Получить("message");          		
	...
	тзВыписка.Колонки.Добавить("iban");
	тзВыписка.Колонки.Добавить("Содержание");
	
	Транзакции = ОтветGetBy_accountКакJson.Получить("mas_tranz");
	
	Для каждого Транзакция Из Транзакции Цикл
		
		НоваяСтрока = тзВыписка.Добавить();
		НоваяСтрока.НомерПП = Транзакция.Получить("doc_number");
		
		Если Транзакция.Получить("priznak_tranz") = "d" Тогда			
			НоваяСтрока.Приход = Транзакция.Получить("sum");  			
		Иначе                                                			
			НоваяСтрока.Расход = Транзакция.Получить("sum");			
		КонецЕсли; 	
		
		НоваяСтрока.ОКПО = Транзакция.Получить("egrpou_kontragent") ;
		НоваяСтрока.Контрагент = Транзакция.Получить("name_kontragent");
		НоваяСтрока.МФО = Транзакция.Получить("mfo_kontragent");
		НоваяСтрока.РСчет = Транзакция.Получить("acc_kontragent");
		НоваяСтрока.iban = Транзакция.Получить("acc_kontragent_iban");
		НоваяСтрока.Содержание = Транзакция.Получить("payment_purpose");
		
	КонецЦикла; 

	Возврат тзВыписка;	
КонецФункции //ИмпортВыписки

// Функция - Перекодировка импорта
// 
// Возвращаемое значение:
//   - 
//
Функция ПерекодировкаИмпорта() Экспорт 
	Возврат Ложь;	
КонецФункции //ПерекодировкаИмпорта
...
&НаСервере
Функция GetAccounts(access_token,idSlave,refresh_token)
	
	idSlave = СтрЗаменить(idSlave, Символы.НПП,"");
	
	Сессия = КоннекторHTTP.СоздатьСессию();
	Сессия.Заголовки.Вставить("Authorization",	access_token);
	Сессия.Заголовки.Вставить("x-clientId",		idSlave);
	
	ПараметрыЗапроса = Новый Соответствие;
	ПараметрыЗапроса.Вставить("client_id",idSlave);
	
	ОтветGetAccountsКакJson = GetJson(
		accounts_addr(),
		ПараметрыЗапроса,
		Неопределено,
		Сессия);
	
	Возврат ОтветGetAccountsКакJson;	
КонецФункции

Функция GetBy_account(ПараметрыДляЗапроса)
	
	// https://api.tas24b.ua/#tag--------
	
	Сессия = КоннекторHTTP.СоздатьСессию();
	Сессия.Заголовки.Вставить("Authorization",	ПараметрыДляЗапроса["access_token"]);
	Сессия.Заголовки.Вставить("x-clientId",		ПараметрыДляЗапроса["x-clientId"]);
	Сессия.Заголовки.Вставить("account_number",	ПараметрыДляЗапроса["account_number"]);
	Сессия.Заголовки.Вставить("currency",		ПараметрыДляЗапроса["currency"]);
	Сессия.Заголовки.Вставить("data_begin",		ПараметрыДляЗапроса["data_begin"]);
	Сессия.Заголовки.Вставить("data_end",		ПараметрыДляЗапроса["data_end"]);
	
	ПараметрыЗапроса = Новый Соответствие;
	ПараметрыЗапроса.Вставить("account_number",	ПараметрыДляЗапроса["account_number"]);
	ПараметрыЗапроса.Вставить("currency",		ПараметрыДляЗапроса["currency"]);
	
	
	ОтветGetBy_accountКакJson = GetJson(
		getby_account_addr(),
		ПараметрыЗапроса,
		Неопределено,
		Сессия);
		
	Возврат ОтветGetBy_accountКакJson;	
	
КонецФункции //GetBy_account

Функция АдресАвторизации() Экспорт 	
	Возврат "https://tas24b.ua/auth-service/auth/tmp/by/phone";	
КонецФункции 

Функция АдресАвторизации_OTP() Экспорт
	Возврат "https://tas24b.ua/auth-service/auth/by/phone_otp";	
КонецФункции

Функция getby_account_addr() Экспорт 
	Возврат "https://tas24b.ua/statements-service/by_account";	
КонецФункции //getby_account

Функция accounts_addr() Экспорт 
	Возврат "https://tas24b.ua/account-service/accounts";
КонецФункции

Функция client_info_addr() Экспорт 
	Возврат "https://tas24b.ua/ekb-service/cb/client_info";
КонецФункции  

Функция refresh() Экспорт 
	Возврат "https://tas24b.ua/auth/by/refresh";
КонецФункции

#КонецОбласти

#Область HTTPConnector
