...
Функция СформироватьПечатнуюФормуЗаявленияОПриеме(МассивОбъектов, ОбъектыПечати)	
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(, Должность = &Должность) КАК ИсточникОтвественныеЛица
...
Функция АдресПрописки(Знач ФизическоеЛицо)
	...
	Если ТипЗнч(КонтактнаяИнформацияОбъекта) = Тип("ТаблицаЗначений") И КонтактнаяИнформацияОбъекта.Количество() > 0 Тогда
		...
		АдресПрописки = КонтактнаяИнформацияОбъекта[0].Представление;
		
	КонецЕсли; 	
	
	Возврат АдресПрописки;
